document.addEventListener('DOMContentLoaded', () => {
  ...
  const normalizeOtp = (value) => String(value || '').normalize('NFKC').replace(/[\u200B-\u200D\uFEFF]/g, '').replace(/[^0-9]/g, '').slice(0, 6);
  const withTimeout = (promise, ms, label) => Promise.race([promise, new Promise((_, reject) => window.setTimeout(() => reject(new Error(`${label} timed out. Please try again.`)), ms))]);
  const api = (path, options = {}) => withTimeout(fetch(`${API_BASE}${path}`, { ...options, headers: { 'Content-Type': 'application/json', 'X-Indo-App-Name': APP_NAME, ...(options.headers || {}) } }), 15000, 'Server request');
  const setSession = (profile) => { localStorage.setItem('indomark.firebaseUser', JSON.stringify(profile)); localStorage.setItem('indomark.user', JSON.stringify(profile)); localStorage.setItem('indomark.session', 'signed-in'); localStorage.setItem('indomark.loginEmail', profile.email); localStorage.removeItem('indomark.pendingName'); };
  const syncProfile = (profile) => { try { const database = window.IndomarkFirebase?.database; if (!database || !profile?.id) return; withTimeout(database.ref(`users/${profile.id}/profile`).set(profile), 5000, 'Profile sync').catch((error) => console.warn('Firebase profile sync failed:', error)); } catch (error) { console.warn('Firebase profile sync setup failed:', error); } };
  ...
  otpInput?.addEventListener('input', () => { const normalized = normalizeOtp(otpInput.value); if (otpInput.value !== normalized) otpInput.value = normalized; });
  ...
  async function beginLogin() {
    ...
    loginButton.disabled = true; showMessage('Checking account…', true);
    try {
      const credential = await withTimeout(firebaseAuth.signInWithEmailAndPassword(email, passwordValue), 15000, 'Firebase login');
      ...
      otpEmail.textContent = email; otpPanel.hidden = false; otpInput.value = ''; showMessage('OTP sent. Check your email to finish login.', true); otpInput.focus();
    } catch (error) {
      const code = String(error?.code || '');
      showMessage(code === 'auth/user-not-found' || code === 'auth/wrong-password' || code === 'auth/invalid-credential' ? 'Invalid email or password.' : (error?.message || 'Login failed.'));
    } finally { loginButton.disabled = false; }
  }
  form?.addEventListener('submit', async (event) => { event.preventDefault(); await beginLogin(); });

  verifyButton?.addEventListener('click', async () => {
    if (!pending?.challengeId) return showMessage('Please request a new OTP.');
    ...
    try {
      ...
      const user = pending.user; if (!user) throw new Error('Login session expired. Please login again.');
      const profile = { id: user.uid, fullName: user.displayName || pending.name || 'Investor', email: user.email || pending.email, active: true };
      setSession(profile); syncProfile(profile);
      showMessage(result.welcomeSent === false ? 'Login successful. Welcome email could not be sent.' : 'Login successful. Welcome email sent.', true);
      window.setTimeout(() => window.location.assign('./home.html'), 350);
    } catch (error) { console.error('Login verification flow failed:', error); showMessage(error?.message || 'Login failed.'); }
