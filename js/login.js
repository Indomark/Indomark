document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      localStorage.setItem('indomark.loginEmail', user.email || email);
      localStorage.setItem('indomark.user', JSON.stringify({ uid: user.uid, fullName: user.displayName || localStorage.getItem('indomark.pendingName') || 'Investor', email: user.email || email }));
      ...
      showMessage(text); if (button) button.disabled = false;
    }
  });
