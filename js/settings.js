(function(){
  ...
  function normalizeBrandText(){
    const replaceInNode=node=>{
      if(node.nodeType===Node.TEXT_NODE){
        if(node.nodeValue&&node.nodeValue.includes('Indomark'))node.nodeValue=node.nodeValue.replace(/Indomark/g,'Indomark');
        return;
      }
  ...
  function lockInactivePage(){
    ...
    installInactiveStyles();document.documentElement.classList.add('account-inactive');
    if(document.getElementById('accountInactiveOverlay'))return;
    const overlay=document.createElement('div');
    overlay.id='accountInactiveOverlay';overlay.setAttribute('role','alertdialog');overlay.setAttribute('aria-modal','true');
    overlay.innerHTML='<div class="account-inactive-box"><h2>Account inactive</h2><p>Your account is currently inactive. App features are unavailable until you activate your account again.</p><a href="profile.html" class="account-inactive-link">Go to Profile</a></div>';
  ...
  function updateAccountStatus(status){
    ...
    if(next==='inactive')lockInactivePage();else removeInactiveLock();
