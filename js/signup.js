document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    if (!window.IndoSpeedFirebase?.auth) return showMessage('Firebase is not available. Refresh and try again.', false);
    const button = form.querySelector('button[type="submit"]');
    if (button) button.disabled = true;
    showMessage('Creating your account…', true);
    try {
      const credential = await window.IndoSpeedFirebase.auth.createUserWithEmailAndPassword(email, passwordValue);
      const user = credential.user;
      await user.updateProfile({ displayName: fullName });
      const profile = { uid: user.uid, fullName, email: user.email || email, createdAt: new Date().toISOString() };
      try { await window.IndoSpeedFirebase.database.ref(`users/${user.uid}/profile`).set(profile); } catch (dbError) { console.warn('Profile database write failed:', dbError); }
      localStorage.setItem('indospeed.user', JSON.stringify(profile));
      ...
      window.setTimeout(() => { window.location.href = './home.html'; }, 400);
    } catch (error) {
      const code = error?.code || '';
      const text = code === 'auth/email-already-in-use' ? 'An account with this email already exists.' : code === 'auth/weak-password' ? 'Password must be at least 6 characters.' : error?.message || 'Account creation failed.';
