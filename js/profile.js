document.addEventListener('DOMContentLoaded',()=>{
 ...
 const renderStatus=(next)=>{
   ...
   if(statusValue){statusValue.textContent=inactive?'Inactive':'Active';statusValue.classList.toggle('inactive',inactive);statusValue.classList.toggle('value',true);}
   if(statusToggle){statusToggle.setAttribute('aria-pressed',String(!inactive));statusToggle.textContent=inactive?'Activate':'Deactivate';statusToggle.classList.toggle('inactive-action',inactive);}
   if(statusHint)statusHint.textContent=inactive?'All app features are disabled until you activate the account again.':'All app features are available.';
 };
 renderStatus(status);
 statusToggle?.addEventListener('click',()=>{
   const current=window.IndoSpeedSettings?.getAccountStatus?.()||'active';
   const next=current==='active'?'inactive':'active';
   window.IndoSpeedSettings?.setAccountStatus?.(next);
   renderStatus(next);
   const message=document.getElementById('profileMessage');
   if(message)message.textContent=next==='inactive'?'Account deactivated. Features are now inactive.':'Account activated. All features are available.';
 });
 window.addEventListener('indospeed-account-status-change',(event)=>renderStatus(event.detail?.status||'active'));

 document.getElementById('logoutBtn')?.addEventListener('click',()=>{localStorage.removeItem('indospeed.user');localStorage.removeItem('indospeed.session');location.href='login.html';});
});
